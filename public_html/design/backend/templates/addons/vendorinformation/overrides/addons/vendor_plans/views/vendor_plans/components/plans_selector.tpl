...
{assign var="currentvendorplan" value=$runtime.company_id|fn_vendor_plans_get_vendor_plan_by_company_id} 
	  
	   {if $currentvendorplan.attributes.price !='0.00' && $currentvendorplan.attributes.price !=''}
	     {if $currentvendorplan.attributes.periodicity =='onetime'}
	     {assign var="datamontly" value="`$currentvendorplan.attributes.price` one time"}
	     {else}
	     {assign var="datamontly" value="`$currentvendorplan.attributes.price` per `$currentvendorplan.attributes.periodicity`"}
	     {/if}
	   
	   {else}
	   {assign var="datamontly" value="no monthly"}
	   {/if}

	   {assign var=datacommission value="`$currentvendorplan.attributes.commission`% commission fee plus VAT"}
	   {if $currentvendorplan.attributes.fixed_commission !='0' && $currentvendorplan.attributes.fixed_commission !=''}
	   {assign var="datafixed" value=" + `$currentvendorplan.attributes.fixed_commission` Per transaction fee"}
	   {else}
	   {assign var="datafixed" value=""}
	   {/if}
	   {assign var="datainfo" value="`$datacommission``$datafixed`"}  
           <div class="vendorplanheading">{__('vendorinformation_vendor_plan_heading', array('[monthly]' => $datamontly))}</div>
           <div class="vendorplantext">{__('vendorinformation_vendor_plan_info', array('[s]' => $datainfo))}</div>
