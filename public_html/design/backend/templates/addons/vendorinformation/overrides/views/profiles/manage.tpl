...
{if $users}
    {capture name="profiles_table"}
        <div class="table-responsive-wrapper longtap-selection">
		   ...
		   {if $auth.user_type == "UserTypes::VENDOR"|enum}
		   <td width="{$person_name_col_width}" class="row-status wrap" data-th="{__("person_name")}">{if $user.firstname || $user.lastname}{$user.lastname} {$user.firstname}{else}-{/if}{if $user.company_id}{include file="views/companies/components/company_name.tpl" object=$user}{/if}</td>
		   {else}
