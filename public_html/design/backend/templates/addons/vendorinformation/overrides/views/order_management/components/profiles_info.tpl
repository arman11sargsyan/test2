...
{capture name="billing_address"}
    ...
    {if !fn_is_empty($user_data)}
        {if $profile_fields.B}
            ...
            {if $user_data.b_phone && $auth.user_type != "UserTypes::VENDOR"|enum}
...
{capture name="shipping_address"}
    ...
    {if !fn_is_empty($user_data)}
        {if $profile_fields.S}
            ...
            {/if}
            {if $user_data.s_address_type && $auth.user_type != "UserTypes::VENDOR"|enum}
...
{capture name="customer_information"}
    {if !fn_is_empty($user_data)}
        <div class="profile-info__name">
            ...
            {if $user_full_name}
                ...
                {elseif $user_data.user_id}
						...
						{else}
							<a href="{"orders.manage&user_id=`$user_data.user_id`"|fn_url}">{$user_full_name}</a>
						{/if}
                {/if}
            {/if}
        </div>
		...
		{/if}
        <div class="clear">
            {if $user_data.phone && $auth.user_type != "UserTypes::VENDOR"|enum}
                <div class="profile-info__phone">
                    <span>{__("phone")}:</span>
                    <a href="tel:{$user_data.phone}"><bdi>{$user_data.phone}</bdi></a>
                </div>
            {/if}
            {if $user_data.fax && $auth.user_type != "UserTypes::VENDOR"|enum}
