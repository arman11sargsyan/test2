{if $category_id}
    {assign var="category_data" value=$category_id|fn_get_category_data:$smarty.const.CART_LANGUAGE:'':false:true:false:true}
...
{if $multiple}
    <tr {if !$clone}id="{$holder}_{$category_id}" {/if}class="cm-js-item {if $clone} cm-clone hidden{/if}">
        {if $position_field}<td width="5%">
            <input type="text" name="{$input_name}[{$category_id}]" value="{math equation="a*b" a=$position b=10}" size="3" class="input-micro"{if $clone} disabled="disabled"{/if} /></td>{/if}
        <td>
            {if !$show_only_name}
