...
<div id="account_info_{$block.snapping_id}">
    ...
    <div class="ty-account-info__buttons buttons-container">
        ...
        {else}
            ...
            <!--Core Change removed dialog to redirect users to auth0 signup-->
            <!-- original code <a href="{if $runtime.controller == "auth" && $runtime.mode == "login_form"}{$config.current_url|fn_url}{else}{"auth.login_form?return_url=`$return_current_url`"|fn_url}{/if}" data-ca-target-id="login_block{$block.snapping_id}" class="cm-dialog-opener cm-dialog-auto-size ty-btn ty-btn__secondary" rel="nofollow">{__("sign_in")}</a>-->
            <a href="{if $runtime.controller == "auth" && $runtime.mode == "login_form"}{$config.current_url|fn_url}{else}{"auth.login_form?return_url=`$return_current_url`"|fn_url}{/if}" data-ca-target-id="login_block{$block.snapping_id}" class="ty-btn ty-btn__secondary" rel="nofollow">{__("sign_in")}</a>
