    ...
    if (Registry::get('runtime.mode') != 'order_webhook')
    {
        ...
        if (count($line_items) > 0)
        {
            ...
            foreach ($line_items as $account_id => $item_arr)
            {
                ...
                // fn_print_r($mapped_shipping_data);
...
function fn_import_products_from_woocommerce($account_data = array() , $string_product_ids = null, $status = 'any')
{
    ...
    try
    {
        ...
        {
            // fn_print_r($string_product_ids);
            // $product_ids = array_map('intval', explode(',', $string_product_ids));
            $product_ids = $string_product_ids;
...
function fn_wk_woocommerce_variation_group_create_products_by_combinations_item($this1, $parent_product_id, $combination_id, $combination, &$product_data)
{
    if (!empty($this1->selected_variants_all))
    {
        ...
        {
            // fn_print_r($product_data,$selected_variants_all[$combination_id]['price'],$selected_variants_all);
            ...
            if(isset($selected_variants_all[$combination_id]['price']) && !empty($selected_variants_all[$combination_id]['price'])){
                $astdata = $selected_variants_all[$combination_id]['price'];
            }
            else{
                $astdata = $product_data['price'];
            }
            $product_data['price'] = fn_format_price_by_currency($astdata, $this1->woocommerce_currency_code, CART_PRIMARY_CURRENCY);
        }
    }
}
...
function fn_get_woocommerce_products_list_for_pagination_data($account_id = 0)
{
    if ($account_id == 0) {
        return;
    }
    $products_list = db_get_hash_array("SELECT * FROM ?:wk_woocommerce_product_map WHERE account_id = ?i",'woocommerce_product_id', $account_id);
