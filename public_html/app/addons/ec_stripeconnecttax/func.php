...
function fn_get_state_descriptions($country_code, $state_code, $lang_code = 'en')
{
	$state_descriptions = db_get_row('SELECT state FROM ?:states INNER JOIN ?:state_descriptions ON ?:states.state_id = ?:state_descriptions.state_id WHERE ?:states.country_code = ?s AND ?:states.code = ?s AND ?:state_descriptions.lang_code =?s',trim($country_code),trim($state_code),$lang_code);
