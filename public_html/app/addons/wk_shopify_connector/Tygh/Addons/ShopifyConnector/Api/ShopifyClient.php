...
use Tygh\Addons\ShopifyConnector\Exceptions\ApiException;
...
class ShopifyClient
{
    ...
    public function call($method, $path, $params = array())
    {
        ...
        if($this->secret === "custom"){
        $request_headers[] = 'X-Shopify-Access-Token: '.$this->api_key;
        }
        else{
        ...
        if (isset($response['errors']) or (!empty($this->last_response_headers['http_status_code']) && $this->last_response_headers['http_status_code'] >= 400)) {
            $this->last_response_headers['http_status_code'] = 200;
            $this->last_response_headers['http_status_message'] = !empty($response['errors']['address']) ? $response['errors']['address'][0] : 'Something went going wrong.';
...
class ShopifyCurlException extends ApiException
{
}
class ShopifyApiException extends ApiException
{
    protected $method;
    protected $path;
    protected $params;
