    ...
    if ($product_id && !empty($product_data)) {
        ...
        foreach ($product_data['options'] as $k => $option) {
            ...
            $feature_category = array (
                'categories_path' => $xmodedata['default_cscart_category_id']
                );            
            
            db_query('UPDATE ?:product_features SET ?u WHERE feature_id = ?i', $feature_category, $feature_id);
        ...
        $getProductsCategoryUpdate = db_get_array('SELECT category_id FROM ?:products_categories WHERE product_id = ?i',$product_id);
        if(!empty($getProductsCategoryUpdate)) {            
            foreach($getProductsCategoryUpdate as $categoryGet) {                
                $getCategoryAssigned = db_get_array('SELECT csart_feature_id FROM ?:wk_shopify_feature_map WHERE product_id = ?i',$product_id);
                
                if(!empty($getCategoryAssigned)) {
                    foreach($getCategoryAssigned as $assigned) {
                        $_array = [
                            'categories_path' => $categoryGet['category_id']
                        ];                        
                        db_query('UPDATE ?:product_features SET ?u WHERE feature_id = ?i',$_array,$assigned['csart_feature_id']);
                    }
                }
            }
        }
...
function fn_wk_create_product_variation($shopId, $product_id = 0, $currency_code = CART_PRIMARY_CURRENCY, $product_data = array(), $option_feature_array = array())
{
    ...
    if ($product_id) {
        ...
        if(false === $pos) {
            ...
            if ($group_id) {
                $result = $service->attachProductsToGroup($group_id, array_keys($request_data));
            } else {
                $result = $service->createGroup(array_merge([$product_id], array_keys($request_data)));
            }
                ...
                foreach ($variation_product_data as $k => $vproduct_data) {
                    ...
                    if (isset($selected_variants_all[$comination_key])) {
                        ...
                        $p_data = array(
                            'product_id' => $k,
                            'shopify_is_parent' => 'N',
                            'price' => $selected_variants_all[$comination_key]['price'],
                        ...
                        $_REQUEST = array();
                        // file_put_contents('product.json',json_encode($p_data));
...
// function fn_wk_shopify_connector_update_product_features_value_pre($product_id, $product_features, $add_new_variant, $lang_code, $params, $category_ids) {
