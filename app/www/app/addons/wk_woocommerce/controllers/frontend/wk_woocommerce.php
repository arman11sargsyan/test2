...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
    ...
    if ($mode == 'product_update_webhook' || $mode == 'product_create_webhook')
    {
        $response = file_get_contents("php://input");
        // file_put_contents('temp_file/product_update_webhook_'.rand().'.txt', $response);
        // file_put_contents('temp_file/product_update_webhook_req_'.rand().'.txt', json_encode($_REQUEST));
        if (isset($_REQUEST['id']) && !empty($_REQUEST['id']))
        {
            $account_id = $_REQUEST['id'];
            $account_data = fn_get_wk_woocommerce_account_data($account_id);
